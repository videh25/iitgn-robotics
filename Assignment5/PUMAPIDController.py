...
class PID_Position_Controller:
    ...
    def achieve_target_state(self):
        while np.linalg.norm(self.error_matrix) > 1e-3:
            self.Manipulator.apply_torques(self.calculate_output_torques())
            print(np.linalg.norm(self.error_matrix))
            print('-------------------------------------')
...
arkoCon = PID_Position_Controller(arko, [1000, 2000, 1000], [0,0,0], [500, 250, 400])
