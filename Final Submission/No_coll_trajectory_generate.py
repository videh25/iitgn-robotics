...
ax.axvline(-(l1 + l2), alpha = 0)
ax.axhline(-ground_height, color = 'g', alpha = 0.1) # Ground Line
...
for x_f,y_f in foot_back_ends:
    no_coll_trajectory.append((x_f, max(y_f, -ground_height)))
    # no_coll_trajectory.append((x_f, y_f))

ax.scatter([x[0] for x in no_coll_trajectory], [x[1] for x in no_coll_trajectory], s = 0.6, color = 'b')
ax.scatter([x[0] for x in foot_back_ends], [x[1] for x in foot_back_ends], s = 0.6, color = 'r')

print(no_coll_trajectory)
print('hahahahahha')
print(foot_back_ends)

preet_hope =[]

for i, ((x_f,y_f), (x_b, y_b))  in enumerate(zip(no_coll_trajectory, foot_front_ends)):
    if y_b <= y_f:
        print(i)
