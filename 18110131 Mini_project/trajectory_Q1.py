...
#Initial States
theta=np.arccos((X[0]**2+Y[0]**2-l1**2-l2**2)/(2*l1*l2))
q10=np.arctan2(Y[0],X[0])-np.arctan2((l2*np.sin(theta)),(l1+l2*np.cos(theta)))
q1_dot0=0
q20=q10+theta
q2_dot0=0
...
def updatePlot(i,Q1,Theta):
    ...
    plt.plot(X,Y, 'k--',linewidth=1) #Plotting trajectory line
...
def dynamicSystem(y,t,X,Y):
    ...
    #Using X and Y to calculate q1 and q2 at that time. Ode solvers dont go based on steps you want so we cant directly use Q1 and theta in this
    x=X[0]+t*(X[-1]-X[0])/steps
    y=Y[0]+t*(Y[-1]-Y[0])/steps

    theta=np.arccos((x**2+y**2-l1**2-l2**2)/(2*l1*l2))
    q1_calc=np.arctan2(y,x)-np.arctan2((l2*np.sin(theta)),(l1+l2*np.cos(theta)))
    q2_calc=theta+q1_calc
    ...
    Tau=np.array([[0.01*(q1_calc-q1)],[0.01*(q2_calc-q2)]])#Input torques. Position control values
    Q_ddot=np.matmul(np.linalg.inv(A),(Tau - P - np.matmul(B,Q_dot)))
...
def outputPart_1():
    ...
    anim.save('18110131_q1_part1.mp4')
...
def outputPart_2():
    ...
    anim.save('18110131_q1_part2.mp4')
...
if __name__=="__main__":
    ...
    while not(userInput == 3):
        print("1. Trajecotory follow using Kinematics\n2. Trajectory follow using Dynamics too\n3. Exit\nEnter index for respective result: ")
